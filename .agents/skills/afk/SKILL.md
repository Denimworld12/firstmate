...
2. **Per harness, after the record exists:**
   ...
   - **Claude, Cursor, OpenCode, omp, Grok, or Codex with `config/supervision-host`**: nothing to launch for `/afk`; go on to the announcement.
     ...
     Where the attended host runs, `/quiet` needs nothing there either: the quiet skill's `quiet-check` decides, and a home without it still launches the daemon below for `/quiet`.
