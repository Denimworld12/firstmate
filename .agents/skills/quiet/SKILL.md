...
On homes that need a daemon, it uses the same token-saving tradeoff as `/afk` without exiting on ordinary chat.

This skill is a thin wrapper for the daemon-backed path.
Its injection, busy/composer guards, classification policy, and reliability properties are owned by the `afk` skill and are identical in daemon-backed quiet mode.
Only the flag's mode and exit rule differ.

## What it does

0. **On Pi or pi-signed, enter nothing: the in-process attended branch already keeps routine wakes off main (the `/afk` skill's "What it does" step 2); tell the captain it is already quiet while present.**
   Otherwise first run `bin/fm-afk-launch.sh quiet-check`.
