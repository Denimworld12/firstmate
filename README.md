...
| `/quiet`           | Keep routine wakes off the captain's conversation while present: on Pi and on a non-Pi home with a [ready attended supervision host](docs/supervision-host.md), this is already the ordinary posture (a paused host reports its pause instead); where no host is ready, `/quiet` starts a daemon that stays active through ordinary chat until `/quiet off` |
