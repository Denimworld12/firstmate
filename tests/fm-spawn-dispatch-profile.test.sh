...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected=$(claude_expected_launch "$launch" --dangerously-skip-permissions)
...
claude_expected_launch() {  # <launch> <permission-flag>
  ...
  printf '%s' "export COMPACT_ADVISER_DISABLE=1; env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude $2 --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' $CLAUDE_CONTROL_CHANNEL_FLAG $quoted"
}
...
test_claude_permission_mode_auto_swaps_only_the_permission_flag() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  expected=$(claude_expected_launch "$launch" '--permission-mode auto')
