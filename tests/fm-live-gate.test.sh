...
test_live_gate_does_not_authorize_an_unmarked_home() {
  # A live guard's opt-in controls whether it runs, not which fleet home it
  # can reach. Structural lab authorization remains required under the gate.
  local path out rc
  path="$TMP_ROOT/unmarked.test.sh"
  {
    printf '#!/usr/bin/env bash\nset -u\n'
    printf '. "%s/tests/lib.sh"\n' "$ROOT"
    printf 'fm_live_gate default-on FM_FAKE_LIVE fmfakeharness\n'
    printf '. "%s/bin/fm-gate-refuse-lib.sh"\n' "$ROOT"
    printf 'fm_refuse_if_gate_agent "%s"\n' "$ROOT"
    printf 'printf "allowed\\n"\n'
  } > "$path"
  chmod +x "$path"
  set +e
  out=$(clean_env NO_MISTAKES_GATE=1 FM_HOME="$ROOT" PATH="$BIN:/usr/bin:/bin" "$path" 2>&1)
  rc=$?
  set -e
  expect_code 3 "$rc" "a live guard must refuse a non-lab home under a gate marker"
  assert_contains "$out" "lab authorization failed" \
    "opening a live guard must not bypass lifecycle refusal for an unmarked home"
  assert_not_contains "$out" allowed "refusal must stop the guard before fleet work"
}
...
test_live_gate_does_not_authorize_an_unmarked_home
pass "the shared live gate does not authorize an unmarked home"
test_every_live_guard_is_wired_to_the_shared_gate
