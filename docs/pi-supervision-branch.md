...
On an opted-in non-Pi home, the supervision host runs the branch beside the primary; [supervision-host.md](supervision-host.md) owns its postures and mechanism.
...
One supervision session runs in two postures, attended and away, and the posture is a file: the away-posture record `state/.afk-contract`, written only by `bin/fm-afk-contract.sh` in the same turn as `/afk` and archived by the return path on the captain's first unmarked message.
...
- Main's standing authority relocates to the branch, and nothing more.
  `fm_lease_forbid_branch` passes the branch actor only for the actions whose guarded script opts in, and only while `bin/fm-afk-contract.sh validate` succeeds on a complete, readable, live record; an archived, incomplete, or invalid record restores the attended refusal byte for byte.
