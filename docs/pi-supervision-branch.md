...
- The in-process branch lives in `.pi/extensions/fm-branch-supervision.ts`, which only a Pi primary ever loads.
  The opted-in non-Pi host has its own runtime ([supervision-host.md](supervision-host.md)).
...
On an opted-in non-Pi home, the supervision host runs the branch beside the primary.
[supervision-host.md](supervision-host.md) owns its postures and mechanism.
