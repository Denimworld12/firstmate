...
{claude,cursor,codex}    For `/quiet`, follow the quiet skill's `quiet-check`: when ready, enter nothing because attended supervision already is quiet; when paused, enter nothing and tell the captain routine wakes reach you until recovery; when not ready, use the daemon-backed path.
{claude,cursor,codex}    A quiet daemon from an earlier entry keeps owning supervision until `/quiet off`.
