...
- `/quiet` needs nothing where the attended host is ready, because the attended posture already is quiet mode; `bin/fm-afk-launch.sh quiet-check` requires the engine, required tools, verified writer, identifiable main session, and a dialog mirror passing the feed's own validation (`bin/fm-host-mirror.sh check`) before a quiet entry refuses the daemon.
  If a prerequisite fails, including a missing, unreadable, or malformed mirror, the attended host passes wakes to main and `/quiet` can use the daemon instead.
...
| The dialog mirror | `bin/fm-host-mirror.sh` | Owns the mirror files, writers, and feed ([The dialog mirror](#the-dialog-mirror)). |
| The captain-outcome drain | `bin/fm-wake-drain.sh` and `bin/fm-branch-outcome.sh` | Present unprocessed captain outcomes and acknowledge them with `mark-processed` ([Captain outcomes](#captain-outcomes)). |
...
An attended wake whose mirror is missing, unreadable, or fails the feed's entry validation (including invalid sequence order or an unterminated record) reaches main with `the dialog mirror could not be read` before any engine turn, and the cursor stays where it was; an away wake never reads the mirror or moves its cursor.
...
3. It runs one bounded engine turn with the branch prompt and the wake message carrying the attended dialog mirror or the away record's read-back.
...
Attended, routine outcomes stay off main while captain outcomes wake it as described above.
Away, captain outcomes wait in the outcome store, and the return brief (`bin/fm-afk-return.sh`) presents them.
...
Every path that cannot finish a wake the engine took hands that wake to main, with one `supervision-host: <why>` line after the close; the host adds no delivery path of its own.
